...
1.url包括协议,网址和请求的资源。
2.301与302的究极无敌区别
* 301 为永久重定向 搜索引擎会刷新使用新的网址 浏览器将缓存旧URL与新URL的映射，则客户端/浏览器将不会尝试请求原始位置，从现在开始使用新位置，除非清除缓存。所以在使用301时要慎重,对于一些浏览器来说,这种缓存是无限期的
* 302 旧网页网址仍存在于搜索引擎数据库中，并始终尝试请求旧位置并对其进行抓取。客户端/浏览器仍将尝试请求原始位置。
